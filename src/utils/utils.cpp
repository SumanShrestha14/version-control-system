...
std::string raw_sha_to_hex(const std::string &raw) {
...
std::vector<TreeEntry> parse_tree_object(const std::string &decompressed) {
    ...
    if (pos == std::string::npos) {
        throw std::runtime_error("Malformed tree object: missing header terminator");
    }
...
std::string mode_to_type(const std::string &mode) {
    ...
    if (mode == "160000") return "commit"; // submodule (gitlink)
